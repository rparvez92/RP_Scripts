...
void DataVsSimPlot_dummy() {
    ...
    if (!fDummy || !fDummy->IsOpen()) { 
        Error("DataVsSimPlot_general","Dummy file not found!"); return; 
    }
    ...
    // ==== DUMMY VARIABLES ====
    double H_gtr_dp_dummy;
    double H_cal_etottracknorm_dummy;
    double H_cer_npeSum_dummy;
    ...
    dummyTree->SetBranchAddress("H.gtr.dp", &H_gtr_dp_dummy);

    dummyTree->SetBranchStatus("H.cal.etottracknorm", 1);
    dummyTree->SetBranchAddress("H.cal.etottracknorm", &H_cal_etottracknorm_dummy);

    dummyTree->SetBranchStatus("H.cer.npeSum", 1);
    dummyTree->SetBranchAddress("H.cer.npeSum", &H_cer_npeSum_dummy);
    ...
    hDummyDelta->SetCanExtend(TH1::kAllAxes);
    ...
    for (Long64_t i=0; i<nData; i++) {
        ...
        if (H_gtr_dp > -8.0 && H_gtr_dp < 8.0 &&
            ...
            H_cer_npeSum > 2) {
            hDataDelta->Fill(H_gtr_dp, 1.0/(18.585*0.9995*0.9985));
        }
    ...
    //hDataDelta->Scale(1.0/(18.585*0.9995*0.9985));
    ...
    std::cout << "Total number of Dummy entries: " << nDummy << std::endl;

    for (Long64_t i=0; i<nDummy; i++) {
        dummyTree->GetEntry(i);

        // CUTS
        if (H_gtr_dp_dummy > -8.0 && H_gtr_dp_dummy < 8.0 &&
            H_cal_etottracknorm_dummy > 0.7 &&
            H_cer_npeSum_dummy > 2) {
            hDummyDelta->Fill(H_gtr_dp_dummy, 2.0/(28.597));
        }
        ...
        //if (H_dc_xp_fp > -35.0 && H_dc_xp_fp < 35.0 &&
            //H_cal_etottracknorm > 0.7 &&
            //H_cer_npeSum > 2) {
            //hDummyXfp->Fill(H_dc_xp_fp);
        //}

    }

    // Number of filled Dummy entries 
    std::cout << "Number of filled Dummy delta entries: " << hDummyDelta -> GetEntries() << std::endl; 
    //std::cout << "Number of filled Dummy xfp entries: " << hDummyXfp -> GetEntries() << std::endl;

    // Normalize dummy by charge*hms_eff
    //hDummyDelta->Scale(1.0/(charge_dummy * hms_eff_dummy));
    //hDummyDelta->Scale(2.0/(28.597));

    //hDummyXfp->Scale(1.0/(charge * hms_eff));



    //Data - Dummy
    hDataSubDummyDelta -> Add(hDataDelta, hDummyDelta, 1.0, -1.0);
    ...
    hDataSubDummyDelta->SetLineColor(kGreen);
