...
//   Both Data and Sim are fit with a Gaussian; report Δμ ± σ(Δμ) on the plot in red.
...
static void ProcessOneRun(int run,
  ...
  for (const auto& dp : dpBins) {
    ...
    for (int i = 0; i < (int)vars.size(); i++) {
      ...
      // Fit ranges (also based on DATA peak window; stable across dp-bins)
      ...
      lat.DrawLatex(0.15, 0.72, Form("#Delta#mu = %.5f #pm %.5f", dmu, dmuErr));
...
void heep_check_dpBinnedOffsets(int run = 0,
                                ...
                                int nFocusBins = 5,
                                bool loopRuns = false,
                                const char* outDirC = "PNGs/dp_binned",
                                const char* csvPathC = "tables/heep_dp_binned_offsets.csv") {
