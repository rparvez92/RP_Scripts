// hodo_calib_qc_batch_for_heep.C
// This is a modified version of hodo_calib_qc_batch.C for heep runs. heep runs are also coin runs but
// instead of electron-pion, it is electron-proton.
// Modification: CTime_ePiCoinTime_ROC2 -> CTime_epCoinTime_ROC2, coinPNGs -> heepPNGs
// HEEP runs filename is just like other COIN filename: skimmed_coin_replay_production_<run>_-1.root
// Need to pass COINRuns as the vector name from runs_vec_heep.hh

// Plots per Spec:
//   HMS  : (1) H_gtr_beta vs H_dc_x_fp; (2) beta_mean & beta_sigma vs run
//   SHMS : (1) P_gtr_beta vs P_dc_x_fp; (2) beta_mean & beta_sigma vs run
//   COIN : (1) H_gtr_beta vs H_dc_x_fp; (2) P_gtr_beta vs P_dc_x_fp; (3) 1D CTime_epCoinTime_ROC2;
...
#include "runs_vec_heep.hh" // HEEP run numbers listed in a vector
...
static void DrawBetaVsXfp(TTree *T, const TString &Spec, int Run) {
  ...
  else if (Spec=="coin") Out = TString::Format("heepPNGs/coin_hms_run%d_beta_vs_xfp.png", Run);
  C->SaveAs(Out);
  delete C; delete H2;
}
...
static void DrawCoinTime1D(TTree *T, int Run){
  ...
  const char *Ct = "CTime_epCoinTime_ROC2";
  TH1D *H1 = new TH1D("H1_CTime","Coincidence Time (ROC2);CTime_epCoinTime_ROC2 (ns);Counts",400,0,100);
  H1->Sumw2();
  ...
  if(PeakBin != 0){cout<<"PeakBin, PeakX: "<<PeakBin<<", "<<PeakX<<endl;}
  ...
  C->SaveAs(TString::Format("heepPNGs/coin_run%d_ctime1D_ROC2.png",Run));
  delete C; delete H1;
}
...
static bool ComputeCoinTimeMetrics(TTree *T, double &Mean, double &Sigma, double &NEntries){
  ...
  TCut Cuts = BuildCuts("coin");
  const char *Ct = "CTime_epCoinTime_ROC2";
  TH1D *H = new TH1D("H1_CtFit",";CTime_epCoinTime_ROC2 (ns);Counts",400,0,100);
  H->Sumw2();
...
static void DrawCoinTimeTrends(const std::vector<int> &Runs,
  ...
  // Show only every 'step'-th run label:
  int step = 2;  // try 5, 10, 20 depending on N
  for (int i = 0; i < N; ++i) {
    if ((i % step) == 0) frame->GetXaxis()->SetBinLabel(i+1, Form("%d", Runs[i]));
    else                 frame->GetXaxis()->SetBinLabel(i+1, "");
  }
  ...
  C->SaveAs("heepPNGs/coin_ctime_trends_ROC2.png");
...
static void ProcessOneRun(const TString &Spec, const TString &RootDir, int Run,
  ...
  if (Spec == "hms") {
    ...
    // Quick way: temporarily enable SHMS branches already on in coin
    {
      ...
      C->SaveAs(TString::Format("heepPNGs/coin_shms_run%d_beta_vs_xfp.png",Run));
      delete C; delete H2;
    }
...
//--------------------------------------------------------------
void hodo_calib_qc_batch_for_heep(const char *Spec="", const char *RootDir="", const char *RunsList=""){
  gROOT->SetBatch(kTRUE);
  gSystem->mkdir("hmsPNGs",  true);
  gSystem->mkdir("shmsPNGs", true);
  //gSystem->mkdir("coinPNGs", true);
  gSystem->mkdir("heepPNGs", true);
