...
static TCut BuildCuts(const TString &Spec) {
  TCut HmsPid = "(H.gtr.dp>-8) && (H.gtr.dp<8) && (H.gtr.beta>0) && (H.gtr.beta<1.2) && (H.cal.etottracknorm>0.7) && (H.cer.npeSum>2.0)";

  TCut ShmsBase = "(P.gtr.dp>-10) && (P.gtr.dp<22) && (P.gtr.beta>0) && (P.gtr.beta<1.2) && (P.cal.etottracknorm<0.8)";
...
static void SetBranchStatusesForSpec(TTree *T, const TString &Spec) {
  ...
  if (Spec == "hms") {
    const char* Vars[] = {
      "H.gtr.dp","H.gtr.beta","H.cal.etottracknorm","H.cer.npeSum",
      "H.dc.x_fp"
    };
    for (auto v : Vars) T->SetBranchStatus(v, 1);
  } else if (Spec == "shms") {
    const char* Vars[] = {
      "P.gtr.dp","P.gtr.beta","P.cal.etottracknorm","P.ngcer.npeSum","P.hgcer.npeSum","P.aero.npeSum","P.gtr.p","P.dc.x_fp"
    };
    for (auto v : Vars) T->SetBranchStatus(v, 1);
  } else if (Spec == "coin") {
    const char* Vars[] = {
      // HMS
      "H.gtr.dp","H.gtr.beta","H.cal.etottracknorm","H.cer.npeSum","H.dc.x_fp",
      // SHMS
      "P.gtr.dp","P.gtr.beta","P.cal.etottracknorm","P.ngcer.npeSum","P.hgcer.npeSum","P.aero.npeSum","P.gtr.p","P.dc.x_fp",
      // Coin time
