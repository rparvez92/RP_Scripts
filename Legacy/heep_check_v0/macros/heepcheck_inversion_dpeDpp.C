// File: heep_check_v0/macros/heepcheck_inversion_dpeDpp.C
//
// Purpose:
//   Fit dpe and dpp (with de0=0, dthe=0, dthp=0 fixed) using
...
//   root -l -b -q 'macros/heepcheck_inversion_dpeDpp.C("one",23839,0)'
//   root -l -b -q 'macros/heepcheck_inversion_dpeDpp.C("all",-1,-1)'
...
struct SolveResult {
  ...
  TMatrixD A;      // design matrix 4x2
  TMatrixD AtA;    // 2x2 normal matrix
  TVectorD Aty;    // 2-vector RHS
  TVectorD xfit;   // fitted (dpe,dpp)

  SolveResult() : y(4), c_dpe(4), c_dpp(4), ypred(4), A(4,2), AtA(2,2), Aty(2), xfit(2) {}
};
...
void FillColumnsDpeDpp(double e0, double the0,
                       double& pe0, double& q0, double& thq0, double& W0,
                       TVectorD& c_dpe, TVectorD& c_dpp) {
...
SolveResult SolveOneInternal(const SolveInput& in) {
  ...
  FillColumnsDpeDpp(in.e0_MeV, in.the0_deg,
                    r.pe0_MeV, r.q0_MeV, r.thq0_deg, r.W0_MeV,
                    r.c_dpe, r.c_dpp);

  // Design matrix A = [c_dpe | c_dpp]
  r.A.ResizeTo(4,2);
  for (int i = 0; i < 4; ++i) {
    r.A(i,0) = r.c_dpe(i);
    r.A(i,1) = r.c_dpp(i);
  ...
  r.AtA.ResizeTo(2,2);
  r.Aty.ResizeTo(2);
  r.AtA = TMatrixD(TMatrixD::kAtA, r.A); // A^T A
  r.Aty = r.A.T() * r.y;

  TDecompLU lu(r.AtA);
  int okDecomp = lu.Decompose() ? 1 : 0;
  if (!okDecomp) {
  ...
  r.ypred.ResizeTo(4);
  for (int i = 0; i < 4; ++i) {
    r.ypred(i) = r.c_dpe(i) * r.dpe + r.c_dpp(i) * r.dpp;
  }
...
void PrintColumn(const TVectorD& c, const std::string& name) {
  std::cout << "\n" << name << " response column (rows: W, Em, Pmz, Pmx)\n";
  std::cout << "Units correspond to one unit of " << name << " = 0.1%\n";
...
void PrintNormalSystem(const TMatrixD& AtA, const TVectorD& Aty) {
  std::cout << "\nNormal-equation system for (dpe, dpp)\n";
...
bool WriteCSVRow(const SolveResult& r, const std::string& outcsv) {
  ...
  fout << "dpeDpp,1,1,1,0,0,data_minus_sim,"
       << r.run << ","
       << r.dp_idx << ","
       << r.dp_label << ","
...
bool HeepCheckSolveOneDpeDpp(
    ...
    const std::string& out_csv      = "results/tables/heepcheck_inversion_dpeDpp.csv",
    bool append_output_csv          = true)
{
  ...
  std::cout << "\n============================================================\n";
  std::cout << "HEEPCHECK INVERSION DPE-DPP\n";
  std::cout << "fit_model       : dpeDpp\n";
  ...
  PrintColumn(r.c_dpe, "dpe");
  PrintColumn(r.c_dpp, "dpp");
  PrintMeasured(in);
    ...
    const std::string& out_csv      = "results/tables/heepcheck_inversion_dpeDpp.csv")
{
  ...
  for (size_t i = 0; i < pairs.size(); ++i) {
    ...
    bool ok = HeepCheckSolveOneDpeDpp(run, dp_idx, bigtable_csv, offsets_csv, out_csv, true);
    if (!ok) {
      all_ok = false;
...
void heepcheck_inversion_dpeDpp(
    const char* mode = "one",
    int run = -1,
    int dp_idx = 0,
    ...
    const char* out_csv      = "results/tables/heepcheck_inversion_dpeDpp.csv")
{
  ...
  if (smode == "one") {
    ...
    HeepCheckSolveOneDpeDpp(run, dp_idx, bigtable_csv, offsets_csv, out_csv, true);
    return;
  }

  if (smode == "all") {
    HeepCheckSolveAllDpeDpp(bigtable_csv, offsets_csv, out_csv);
    return;
  }
