// File: heep_check_v0/macros/plot_heepcheck_inversion_dpeDpp.C
//
// Purpose:
//   Make plots from:
...
//     results/PNGs/kin_offsets_dpeDpp/kin_offsets_vs_run_<dp_idx>.png
//
//   Second set (one plot per run):
//     results/PNGs/kin_offsets_dpeDpp/kin_offsets_vs_dpbinCenter_<run>.png
//
// Plot choices:
...
bool PlotOneDpIdx(const std::vector<Row>& rows, int dp_idx, const std::string& outdir) {
  ...
  TCanvas* c = new TCanvas(Form("c_run_dpeDpp_%d", dp_idx),
...
bool PlotOneRunDpCenter(const std::vector<Row>& rows, int run, const std::string& outdir) {
  ...
  TCanvas* c = new TCanvas(Form("c_dpcenter_dpeDpp_%d", run),
...
void plot_heepcheck_inversion_dpeDpp(
    const char* input_csv = "results/tables/heepcheck_inversion_dpeDpp.csv",
    const char* out_dir   = "results/PNGs/kin_offsets_dpeDpp/")
{
  gROOT->SetBatch(kTRUE);
  gStyle->SetOptStat(0);

  const std::string incsv = input_csv ? input_csv : "results/tables/heepcheck_inversion_dpeDpp.csv";
  const std::string outdir = out_dir ? out_dir : "results/PNGs/kin_offsets_dpeDpp/";
