// File: heep_check_v0/macros/plot_heepcheck_inversion_dpeDppDthp.C
//
// Purpose:
//   Make plots from:
...
//     results/PNGs/kin_offsets_dpeDppDthp/kin_offsets_vs_run_<dp_idx>.png
//
//   Second set (one plot per run):
//     results/PNGs/kin_offsets_dpeDppDthp/kin_offsets_vs_dpbinCenter_<run>.png
//
// Plot choices:
//   - same 4-panel format as before
//   - markers only
//   - no legend
//   - grid on
//   - auto y-range
...
bool LoadCSV(const std::string& path, std::vector<Row>& rows) {
  ...
  const std::vector<std::string> required = {
    "fit_model","de0_fixed","dthe_fixed","dthp_fixed","dpe_fixed","dpp_fixed","sign_convention",
  ...
  while (std::getline(fin, line)) {
    ...
    ToInt(tok[idx.at("dpe_fixed")], r.dpe_fixed);
...
bool PlotOneDpIdx(const std::vector<Row>& rows, int dp_idx, const std::string& outdir) {
  ...
  TCanvas* c = new TCanvas(Form("c_run_dpeDppDthp_%d", dp_idx),
...
bool PlotOneRunDpCenter(const std::vector<Row>& rows, int run, const std::string& outdir) {
  ...
  TCanvas* c = new TCanvas(Form("c_dpcenter_dpeDppDthp_%d", run),
...
void plot_heepcheck_inversion_dpeDppDthp(
    const char* input_csv = "results/tables/heepcheck_inversion_dpeDppDthp.csv",
    const char* out_dir   = "results/PNGs/kin_offsets_dpeDppDthp/")
{
  gROOT->SetBatch(kTRUE);
  gStyle->SetOptStat(0);

  const std::string incsv = input_csv ? input_csv : "results/tables/heepcheck_inversion_dpeDppDthp.csv";
  const std::string outdir = out_dir ? out_dir : "results/PNGs/kin_offsets_dpeDppDthp/";
