// File: heep_check_v0/macros/heepcheck_inversion_dpeOnly.C
//
// Purpose:
//   Fit only dpe (with de0=0, dthe=0, dthp=0, dpp=0 fixed) using
...
//   - e0 = ebeam * 1000 [MeV]
//   - the0 = shms_th [deg]
//   - dpe is reported in heepcheck units: [0.1%]
//
// Suggested usage from heep_check_v0/:
//   root -l -b -q 'macros/heepcheck_inversion_dpeOnly.C("one",23839,0)'
//   root -l -b -q 'macros/heepcheck_inversion_dpeOnly.C("all",-1,-1)'
...
struct SolveResult {
  ...
  SolveResult() : y(4), c_dpe(4), ypred(4) {}
};
...
void FillDpeColumn(double e0, double the0,
  ...
  const double dpe_phys = 0.001 * pe0; // +0.1%
  c_dpe(0) = Wfunc(e0, pe0 + dpe_phys, the0) - W0;                // W
  c_dpe(1) = -dpe_phys;                                            // Em
  c_dpe(2) =  dpe_phys * std::cos((the0 + thq0) * kRadFac);        // Pmz
  c_dpe(3) =  dpe_phys * std::sin((the0 + thq0) * kRadFac);        // Pmx
}
...
SolveResult SolveOneInternal(const SolveInput& in) {
  ...
  FillDpeColumn(in.e0_MeV, in.the0_deg,
  ...
  double denom = 0.0;
  for (int i = 0; i < 4; ++i) {
    numer += r.c_dpe(i) * r.y(i);
    denom += r.c_dpe(i) * r.c_dpe(i);
  }

  if (std::fabs(denom) < 1e-30) {
    std::cerr << "[ERROR] dpe-only denominator is too small for run=" << in.run
              << ", dp_idx=" << in.dp_idx << "\n";
    r.ok = false;
    return r;
  }
...
void PrintColumnDpe(const TVectorD& c) {
  std::cout << "\ndpe response column (rows: W, Em, Pmz, Pmx)\n";
  std::cout << "Units correspond to one unit of dpe = 0.1%\n";
...
bool WriteCSVRow(const SolveResult& r, const std::string& outcsv) {
  ...
  if (needHeader) {
    fout << "fit_model,de0_fixed,dthe_fixed,dthp_fixed,dpp_fixed,sign_convention,"
  ...
  fout << "dpeOnly,1,1,1,1,data_minus_sim,"
       << r.run << ","
       << r.dp_idx << ","
       << r.dp_label << ","
...
bool HeepCheckSolveOneDpeOnly(
    ...
    const std::string& out_csv      = "results/tables/heepcheck_inversion_dpeOnly.csv",
    bool append_output_csv          = true)
{
  ...
  std::cout << "\n============================================================\n";
  std::cout << "HEEPCHECK INVERSION DPE-ONLY\n";
  std::cout << "fit_model       : dpeOnly\n";
  std::cout << "de0_fixed       : 1\n";
  std::cout << "dthe_fixed      : 1\n";
  std::cout << "dthp_fixed      : 1\n";
  std::cout << "dpp_fixed       : 1\n";
    ...
    const std::string& out_csv      = "results/tables/heepcheck_inversion_dpeOnly.csv")
{
  ...
  for (size_t i = 0; i < pairs.size(); ++i) {
    ...
    bool ok = HeepCheckSolveOneDpeOnly(run, dp_idx, bigtable_csv, offsets_csv, out_csv, true);
    if (!ok) {
      all_ok = false;
...
void heepcheck_inversion_dpeOnly(
    const char* mode = "one",
    int run = -1,
    int dp_idx = 0,
    ...
    const char* out_csv      = "results/tables/heepcheck_inversion_dpeOnly.csv")
{
  ...
  if (smode == "one") {
    ...
    HeepCheckSolveOneDpeOnly(run, dp_idx, bigtable_csv, offsets_csv, out_csv, true);
    return;
  }

  if (smode == "all") {
    HeepCheckSolveAllDpeOnly(bigtable_csv, offsets_csv, out_csv);
    return;
  }
