...
namespace {
  ...
  // Holds only the values needed by this macro for each run
  struct BigTableEntry {
    double charge_mC_from_BCM4A;  // ≡ BCM4A_Q
    double hms_efficiency;        // ≡ h_esing_Eff
    double prescale_factor_ps5;   // ≡ ps5
  };

  // Maps run number to BigTableEntry
  std::unordered_map<int, BigTableEntry> g_bigtable_map;

  bool g_bigtable_has_been_loaded = false;

  // ---------------------------------------------------------------
  // Load the bigtable from CSV *once* and store in g_bigtable_map
  // ---------------------------------------------------------------
  void LoadBigTableCSV(const std::string& csv_path = "rsidis_bigtable_pass0.csv")
  {
    if (g_bigtable_has_been_loaded) return;
    ...
    // Helper to find which column index corresponds to a given name
    ...
    const int idx_charge_BCM4A    = find_col_index("BCM4A_Q");
    const int idx_hms_efficiency  = find_col_index("h_esing_Eff");
    const int idx_prescale_ps5    = find_col_index("ps5");

    if (idx_run < 0 || idx_charge_BCM4A < 0 || idx_hms_efficiency < 0 || idx_prescale_ps5 < 0) {
      std::cerr << "[ERROR] One or more required columns are missing in bigtable CSV!\n"
                   "Required columns: run, BCM4A_Q, h_esing_Eff, ps5\n";
      return;
    }

    // ---------------------------------------------------------
    // Read each data row and fill g_bigtable_map
    // ---------------------------------------------------------
    std::string line;
    while (std::getline(infile, line)) {
      ...
      int max_idx = std::max({idx_run, idx_charge_BCM4A, idx_hms_efficiency, idx_prescale_ps5});
      if ((int)fields.size() <= max_idx) continue;

      int run_number = std::stoi(fields[idx_run]);

      BigTableEntry entry;
      entry.charge_mC_from_BCM4A = std::stod(fields[idx_charge_BCM4A]);
      entry.hms_efficiency       = std::stod(fields[idx_hms_efficiency]);
      entry.prescale_factor_ps5  = std::stod(fields[idx_prescale_ps5]);

      g_bigtable_map[run_number] = entry;
    }

    g_bigtable_has_been_loaded = true;
...
static std::string DnDRootPath(int run) {
  ...
  return Form("./Pass0_ROOTfiles/coin_replay_production_%d_-1.root", run);
}
static std::string DnDReportPath(int run) {
  //return Form("./REPORT_OUTPUT/COIN/PRODUCTION/replay_coin_production_%d_-1.report", run);
  return Form("./Pass0_REPORT_OUTPUT/COIN/PRODUCTION/replay_coin_production_%d_-1.report", run);
}
...
*/
    ...
    // -----------------------------------------------
    // Replace REPORT_OUTPUT access with bigtable access
    // -----------------------------------------------
    const BigTableEntry* row = GetBigTableEntry(runNumber);
    if (!row) {
      std::cerr << "[ERROR] Missing bigtable entry for run " << runNumber << std::endl;
      return nullptr;  // Skip this run
    }

    // Accumulate charge normalization
    Qsum_mC += row->charge_mC_from_BCM4A;

    // Debugging printout
    std::cout << "[RUN " << runNumber << "] "
              << " charge_mC = " << row->charge_mC_from_BCM4A
              << " hms_eff = " << row->hms_efficiency
              << " ps_factor = " << row->prescale_factor_ps5
              << std::endl;

    // Sanity checks
    if (row->charge_mC_from_BCM4A <= 0 || row->hms_efficiency <= 0 || row->prescale_factor_ps5 <= 0) {
      std::cerr << "[WARNING] Non-positive parameter in bigtable for run "
                << runNumber << std::endl;
    }

    // Scale factor = ps5 / hms_eff
    TCut dnd_scale = Form("%f / (%f)",
                          row->prescale_factor_ps5,
                          row->hms_efficiency);
...
void DataVsSimPlot_MultiDataMultiDummy() {
    ...
    std::vector<int> dataRuns = {24329, 24330};
    //std::vector<int> dummyRuns = {24335, 24336, 24337, 24338};
    std::vector<int> dummyRuns = {24335, 24336};
