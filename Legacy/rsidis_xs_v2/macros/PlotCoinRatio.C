...
namespace {
      ...
      std::ofstream& log)
  {
    ...
    auto hSig    = BuildYield(runsSignal,    dataExpr, "hSig",    nbins, xmin, xmax, baseCutsBool, qSig,    infoMap, enabled, coinCache, log);
    auto hDum    = BuildYield(runsDummy,     dataExpr, "hDum",    nbins, xmin, xmax, baseCutsBool, qDum,    infoMap, enabled, coinCache, log);
    auto hPosSig = BuildYield(runsPosSignal, dataExpr, "hPosSig", nbins, xmin, xmax, baseCutsBool, qPosSig, infoMap, enabled, coinCache, log);
    auto hPosDum = BuildYield(runsPosDummy,  dataExpr, "hPosDum", nbins, xmin, xmax, baseCutsBool, qPosDum, infoMap, enabled, coinCache, log);
...
void PlotCoinRatio(const char* manifestPath, const char* outputDir)
{
  ...
  log << "plot_var=W nbins=" << nbins << " xmin=" << xmin << " xmax=" << xmax << "\n";
  log << "plot_var=xbj nbins=" << 30 << " xmin=" << 0.0 << " xmax=" << 1.0 << "\n";
