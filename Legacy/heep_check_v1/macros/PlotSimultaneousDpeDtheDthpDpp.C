// PlotSimultaneousDpeDtheDthpDpp.C
//
// Read a simultaneous-fit CSV produced by:
//   macros/SimultaneousDpeDtheDthpDpp.C
//
// and produce, for each setting, three PNGs:
//
//   results/PNGs/PlotSimultaneousDpeDtheDthpDpp/<Constrained,Unconstrained>/setting<A,B>_kinOffsets.png
//   results/PNGs/PlotSimultaneousDpeDtheDthpDpp/<Constrained,Unconstrained>/setting<A,B>_residuals.png
//   results/PNGs/PlotSimultaneousDpeDtheDthpDpp/<Constrained,Unconstrained>/setting<A,B>_chi2ndf.png
//
// Plot types:
...
//   - the CSV stores bin-specific dthe, dpe, dthp, and dpp columns
//     (for example dthe_b1_fit, dpe_b1_fit, dthp_b1_fit, dpp_b1_fit),
//     which are mapped here onto the plotted series row-by-row
//
// Default mode-specific paths:
//   constrained:
//     results/tables/SimultaneousDpeDtheDthpDpp_constrained.csv
//     results/PNGs/PlotSimultaneousDpeDtheDthpDpp/Constrained
//
//   unconstrained:
//     results/tables/SimultaneousDpeDtheDthpDpp_unconstrained.csv
//     results/PNGs/PlotSimultaneousDpeDtheDthpDpp/Unconstrained
//
// Example:
//   root -l -b -q 'macros/PlotSimultaneousDpeDtheDthpDpp.C("constrained")'
//   root -l -b -q 'macros/PlotSimultaneousDpeDtheDthpDpp.C("unconstrained")'
//   root -l -b -q 'macros/PlotSimultaneousDpeDtheDthpDpp.C("constrained","results/tables/SimultaneousDpeDtheDthpDpp_constrained.csv","results/PNGs/PlotSimultaneousDpeDtheDthpDpp/Constrained")'
...
static void MakeOffsetsCanvas(const std::string& setting,
  ...
  TCanvas* c = new TCanvas(Form("c_sim_dpdtpdpp_kin_%s_%s", setting.c_str(), modeLabel.c_str()),
                           "SimultaneousDpeDtheDthpDpp kinematic offsets", 1200, 900);
...
static void MakeResidualsCanvas(const std::string& setting,
                                ...
                                const std::string& outDir,
                                const std::string& modeLabel) {
  TCanvas* c = new TCanvas(Form("c_sim_dpdtpdpp_res_%s_%s", setting.c_str(), modeLabel.c_str()),
                           "SimultaneousDpeDtheDthpDpp residuals", 1200, 900);
  c->Divide(2,2,0.002,0.002);
...
static void MakeChi2Canvas(const std::string& setting,
                           ...
                           const std::string& outDir,
                           const std::string& modeLabel) {
  TCanvas* c = new TCanvas(Form("c_sim_dpdtpdpp_chi2_%s_%s", setting.c_str(), modeLabel.c_str()),
                           "SimultaneousDpeDtheDthpDpp chi2/ndf", 900, 700);
  c->cd();
  gPad->SetGrid(1,1);
  gPad->SetTicks(1,1);
...
void PlotSimultaneousDpeDtheDthpDpp(const char* modeIn = "unconstrained",
                                    const char* inCsvIn = "",
                                    const char* outDirIn = "") {
  ...
  if (inCsv.empty()) {
    inCsv = "results/tables/SimultaneousDpeDtheDthpDpp_" + mode + ".csv";
  }

  std::string outDir = Trim(outDirIn ? outDirIn : "");
  if (outDir.empty()) {
    outDir = "results/PNGs/PlotSimultaneousDpeDtheDthpDpp/" + modeDir;
  }
  ...
  std::cout << "[INFO] Wrote PlotSimultaneousDpeDtheDthpDpp PNGs to " << outDir
