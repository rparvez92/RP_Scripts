// PlotSimultaneousDpeDthp.C
//
// Read a simultaneous-fit CSV produced by:
//   macros/SimultaneousDpeDthp.C
//
// and produce, for each setting, three PNGs:
//
//   results/PNGs/PlotSimultaneousDpeDthp/<Constrained,Unconstrained>/setting<A,B>_kinOffsets.png
//   results/PNGs/PlotSimultaneousDpeDthp/<Constrained,Unconstrained>/setting<A,B>_residuals.png
//   results/PNGs/PlotSimultaneousDpeDthp/<Constrained,Unconstrained>/setting<A,B>_chi2ndf.png
//
// Plot types:
...
//   - the CSV stores bin-specific dpe and dthp columns
//     (for example dpe_b1_fit, dthp_b1_fit), which are mapped here
//     onto the plotted series row-by-row
//
// Default mode-specific paths:
//   constrained:
//     results/tables/SimultaneousDpeDthp_constrained.csv
//     results/PNGs/PlotSimultaneousDpeDthp/Constrained
//
//   unconstrained:
//     results/tables/SimultaneousDpeDthp_unconstrained.csv
//     results/PNGs/PlotSimultaneousDpeDthp/Unconstrained
//
// Example:
//   root -l -b -q 'macros/PlotSimultaneousDpeDthp.C("constrained")'
//   root -l -b -q 'macros/PlotSimultaneousDpeDthp.C("unconstrained")'
//   root -l -b -q 'macros/PlotSimultaneousDpeDthp.C("constrained","results/tables/SimultaneousDpeDthp_constrained.csv","results/PNGs/PlotSimultaneousDpeDthp/Constrained")'
...
static void MakeOffsetsCanvas(const std::string& setting,
  ...
  TCanvas* c = new TCanvas(Form("c_sim_dpdthp_kin_%s_%s", setting.c_str(), modeLabel.c_str()),
                           "SimultaneousDpeDthp kinematic offsets", 1200, 900);
...
static void MakeResidualsCanvas(const std::string& setting,
                                ...
                                const std::string& outDir,
                                const std::string& modeLabel) {
  TCanvas* c = new TCanvas(Form("c_sim_dpdthp_res_%s_%s", setting.c_str(), modeLabel.c_str()),
                           "SimultaneousDpeDthp residuals", 1200, 900);
  c->Divide(2,2,0.002,0.002);
...
static void MakeChi2Canvas(const std::string& setting,
                           ...
                           const std::string& outDir,
                           const std::string& modeLabel) {
  TCanvas* c = new TCanvas(Form("c_sim_dpdthp_chi2_%s_%s", setting.c_str(), modeLabel.c_str()),
                           "SimultaneousDpeDthp chi2/ndf", 900, 700);
  c->cd();
  gPad->SetGrid(1,1);
  gPad->SetTicks(1,1);
...
void PlotSimultaneousDpeDthp(const char* modeIn = "unconstrained",
                             const char* inCsvIn = "",
                             const char* outDirIn = "") {
  ...
  if (inCsv.empty()) {
    inCsv = "results/tables/SimultaneousDpeDthp_" + mode + ".csv";
  }

  std::string outDir = Trim(outDirIn ? outDirIn : "");
  if (outDir.empty()) {
    outDir = "results/PNGs/PlotSimultaneousDpeDthp/" + modeDir;
  }
  ...
  std::cout << "[INFO] Wrote PlotSimultaneousDpeDthp PNGs to " << outDir
