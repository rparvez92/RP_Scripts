/*
  ...
  dpe_b1 through dpe_b5, dthp_b1 through dthp_b5, and the corresponding full
  ...
  Run examples:
    root -l -b -q 'macros/SimultaneousDpeDthp.C("results/tables/OffsetsSummaryBySetting.csv","","unconstrained")'
    root -l -b -q 'macros/SimultaneousDpeDthp.C("results/tables/OffsetsSummaryBySetting.csv","","constrained")'
*/
...
static void SimultaneousChi2Fcn(Int_t& /*npar*/, Double_t* /*grad*/, Double_t& fval,
  ...
  double chi2 = 0.0;
  const int nbin = static_cast<int>(gFitContext.dpe_param_index_by_bin.size());
  const int idx_dthe = 0;
  const int idx_dpp  = 1 + 2 * nbin;
...
static bool RunSettingFit(const std::string& setting,
  ...
  for (int i = 0; i < nbin; ++i) {
    ...
    gFitContext.dthp_param_index_by_bin[i] = 1 + nbin + i;
  }
  ...
  for (int ibin = 0; ibin < nbin; ++ibin) {
    ...
    const int idx_dthp = 1 + nbin + ibin;
    ...
    if (setting == "A") {
      ...
      bout.dPmy_pred =  4.10  * res.dthe +  5.75 * bout.dpe - 2.27 * bout.dthp;
      ...
      bout.dPmy_pred =  3.69  * res.dthe +  4.30 * bout.dpe - 3.74 * bout.dthp;
...
static void WriteHeader(std::ofstream& fout, const std::vector<std::string>& param_names) {
  ...
  for (const auto& name : param_names) {
    if (name == "dthe" || name == "dpp") continue;
    fout << name << "_fit," << name << "_fit_err,";
  }
  fout << "dthe_fit,dthe_fit_err,dpp_fit,dpp_fit_err,";
...
void SimultaneousDpeDthp(const char* inCsv = "results/tables/OffsetsSummaryBySetting.csv",
                         const char* outCsv = "",
  ...
  if (outPath.empty()) {
    outPath = "results/tables/SimultaneousDpeDthp_" + mode + ".csv";
  }
