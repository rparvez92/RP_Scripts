/*
  ...
  and perform a simultaneous fit, per setting, under the hypothesis that:
    ...
    - dthe is common to all fitted narrow bins in that setting
  ...
  dpe_b1 through dpe_b5 and the corresponding full covariance block; fields not
  used by a given setting (for example dpe_b5 for Setting A) are written as nan.
  ...
  Run examples:
    root -l -b -q 'macros/SimultaneousDpeOnly.C("results/tables/OffsetsSummaryBySetting.csv","","unconstrained")'
    root -l -b -q 'macros/SimultaneousDpeOnly.C("results/tables/OffsetsSummaryBySetting.csv","","constrained")'
*/
...
static std::vector<std::string> GlobalParamNames() {
  return {"dthe", "dpe_b1", "dpe_b2", "dpe_b3", "dpe_b4", "dpe_b5", "dthp", "dpp"};
}
...
static void SimultaneousChi2Fcn(Int_t& /*npar*/, Double_t* /*grad*/, Double_t& fval,
  ...
  double chi2 = 0.0;
  const int idx_dthe = 0;
  const int idx_dthp = static_cast<int>(gFitContext.dpe_param_index_by_bin.size()) + 1;
...
static bool RunSettingFit(const std::string& setting,
  ...
  gFitContext.dpe_param_index_by_bin.resize(bins.size());
  for (int i = 0; i < static_cast<int>(bins.size()); ++i) {
    gFitContext.dpe_param_index_by_bin[i] = 1 + i;
  }
  ...
  const int idx_dthe = 0;
  const int idx_dthp = npar - 2;
  const int idx_dpp = npar - 1;
  ...
  res.dthe = res.params[idx_dthe];
  res.dthe_err = res.param_errs[idx_dthe];
  ...
  if (constrained) {
    bool onLimit = atBound(res.dthe) || atBound(res.dthp) || atBound(res.dpp);
    for (int i = 0; i < static_cast<int>(bins.size()); ++i) onLimit = onLimit || atBound(res.params[1 + i]);
    if (onLimit) res.fit_note += "_parameter_at_limit";
  }

  for (int ibin = 0; ibin < static_cast<int>(bins.size()); ++ibin) {
    const BinData& b = byBin.at(bins[ibin]);
    const int idx_dpe = 1 + ibin;
    FitBinResult bout;
    bout.bin = bins[ibin];
    ...
    if (setting == "A") {
      bout.dW_pred   = -14.08 * res.dthe + -8.62 * bout.dpe;
      bout.dEm_pred  = -7.06  * bout.dpe + -2.10 * res.dpp;
      bout.dPmz_pred = -5.75  * res.dthe +  4.10 * bout.dpe + 2.27 * res.dpp;
      bout.dPmy_pred =  4.10  * res.dthe +  5.75 * bout.dpe - 2.27 * res.dthp;
      ...
      bout.dW_pred   = -17.33 * res.dthe + -8.62 * bout.dpe;
      bout.dEm_pred  = -5.66  * bout.dpe + -3.63 * res.dpp;
      bout.dPmz_pred = -4.30  * res.dthe +  3.69 * bout.dpe + 3.74 * res.dpp;
      bout.dPmy_pred =  3.69  * res.dthe +  4.30 * bout.dpe - 3.74 * res.dthp;
...
static void WriteHeader(std::ofstream& fout, const std::vector<std::string>& param_names) {
  ...
  fout << "dthe_fit,dthe_fit_err,";
  for (const auto& name : param_names) {
    if (name == "dthe" || name == "dthp" || name == "dpp") continue;
...
void SimultaneousDpeOnly(const char* inCsv = "results/tables/OffsetsSummaryBySetting.csv",
                         const char* outCsv = "",
  ...
  if (outPath.empty()) {
    outPath = "results/tables/SimultaneousDpeOnly_" + mode + ".csv";
  }
