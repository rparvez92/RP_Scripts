/*
  ...
  Run examples:
    root -l -b -q 'macros/SimultaneousDpeDthe.C("results/tables/OffsetsSummaryBySetting.csv","","unconstrained")'
    root -l -b -q 'macros/SimultaneousDpeDthe.C("results/tables/OffsetsSummaryBySetting.csv","","constrained")'
*/
...
static bool RunSettingFit(const std::string& setting,
  ...
  if (constrained) {
    bool onLimit = atBound(res.dthp) || atBound(res.dpp);
    for (int i = 0; i < npar - 2; ++i) onLimit = onLimit || atBound(res.params[i]);
  ...
  for (int ibin = 0; ibin < nbin; ++ibin) {
    ...
    if (setting == "A") {
      ...
      bout.dPmy_pred =  4.10  * bout.dthe +  5.75 * bout.dpe - 2.27 * res.dthp;
      ...
      bout.dPmy_pred =  3.69  * bout.dthe +  4.30 * bout.dpe - 3.74 * res.dthp;
...
void SimultaneousDpeDthe(const char* inCsv = "results/tables/OffsetsSummaryBySetting.csv",
                         const char* outCsv = "",
  ...
  if (outPath.empty()) {
    outPath = "results/tables/SimultaneousDpeDthe_" + mode + ".csv";
  }
