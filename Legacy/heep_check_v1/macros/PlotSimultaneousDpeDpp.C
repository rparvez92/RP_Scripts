// PlotSimultaneousDpeDpp.C
//
// Read a simultaneous-fit CSV produced by:
//   macros/SimultaneousDpeDpp.C
//
// and produce, for each setting, three PNGs:
//
//   results/PNGs/PlotSimultaneousDpeDpp/<Constrained,Unconstrained>/setting<A,B>_kinOffsets.png
//   results/PNGs/PlotSimultaneousDpeDpp/<Constrained,Unconstrained>/setting<A,B>_residuals.png
//   results/PNGs/PlotSimultaneousDpeDpp/<Constrained,Unconstrained>/setting<A,B>_chi2ndf.png
//
// Plot types:
...
//   - the CSV stores bin-specific dpe and dpp columns
//     (for example dpe_b1_fit, dpp_b1_fit), which are mapped here
//     onto the plotted series row-by-row
//
// Default mode-specific paths:
//   constrained:
//     results/tables/SimultaneousDpeDpp_constrained.csv
//     results/PNGs/PlotSimultaneousDpeDpp/Constrained
//
//   unconstrained:
//     results/tables/SimultaneousDpeDpp_unconstrained.csv
//     results/PNGs/PlotSimultaneousDpeDpp/Unconstrained
//
// Example:
//   root -l -b -q 'macros/PlotSimultaneousDpeDpp.C("constrained")'
//   root -l -b -q 'macros/PlotSimultaneousDpeDpp.C("unconstrained")'
//   root -l -b -q 'macros/PlotSimultaneousDpeDpp.C("constrained","results/tables/SimultaneousDpeDpp_constrained.csv","results/PNGs/PlotSimultaneousDpeDpp/Constrained")'
...
static std::vector<Row> ReadSimultaneousCsv(const std::string& csvPath) {
  ...
  while (std::getline(fin, line)) {
    ...
    r.dpp_fit = getD(idx("dpp_" + binLower + "_fit"));
    r.dpp_fit_err = getD(idx("dpp_" + binLower + "_fit_err"));
...
static void MakeOffsetsCanvas(const std::string& setting,
  ...
  TCanvas* c = new TCanvas(Form("c_sim_dpdpp_kin_%s_%s", setting.c_str(), modeLabel.c_str()),
                           "SimultaneousDpeDpp kinematic offsets", 1200, 900);
...
static void MakeResidualsCanvas(const std::string& setting,
                                ...
                                const std::string& outDir,
                                const std::string& modeLabel) {
  TCanvas* c = new TCanvas(Form("c_sim_dpdpp_res_%s_%s", setting.c_str(), modeLabel.c_str()),
                           "SimultaneousDpeDpp residuals", 1200, 900);
  c->Divide(2,2,0.002,0.002);
...
static void MakeChi2Canvas(const std::string& setting,
                           ...
                           const std::string& outDir,
                           const std::string& modeLabel) {
  TCanvas* c = new TCanvas(Form("c_sim_dpdpp_chi2_%s_%s", setting.c_str(), modeLabel.c_str()),
                           "SimultaneousDpeDpp chi2/ndf", 900, 700);
  c->cd();
  gPad->SetGrid(1,1);
  gPad->SetTicks(1,1);
...
void PlotSimultaneousDpeDpp(const char* modeIn = "unconstrained",
                            const char* inCsvIn = "",
                            const char* outDirIn = "") {
  ...
  if (inCsv.empty()) {
    inCsv = "results/tables/SimultaneousDpeDpp_" + mode + ".csv";
  }

  std::string outDir = Trim(outDirIn ? outDirIn : "");
  if (outDir.empty()) {
    outDir = "results/PNGs/PlotSimultaneousDpeDpp/" + modeDir;
  }
  ...
  std::cout << "[INFO] Wrote PlotSimultaneousDpeDpp PNGs to " << outDir
