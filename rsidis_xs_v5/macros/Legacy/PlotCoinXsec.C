                  ...
                  bool saveTables = true)
{
  ...
  if (saveTables) {
    ...
    const std::string fitHeader =
      "mode,group_id,curve_label,setting_id,pt_bin,z_bin,pt_lo,pt_hi,pt_center,z_lo,z_hi,z_center,n_points,"
      "M0,M0_err,A,A_err,B,B_err,chi2,ndf,prob,fit_tier";
    // Overwrite each run so re-running the macro does NOT duplicate rows
    fitcsv.open(fitPath, std::ios::out);
    fitcsv << fitHeader << std::endl;
    log << "Fit CSV: " << fitPath << std::endl;
  }
  if (saveTables) {
  ...
  for (int iPt=0; iPt<nPt; iPt++) {
    ...
    for (const auto& gp : graphs) {
      ...
      const double zLo = binning.zEdges[gp.zBin];
      const double zHi = binning.zEdges[gp.zBin + 1];

      // --- style per z ---
      const Color_t col = zColors[gp.zBin % nZColors];
      ...
      // --- draw points with error bars (vertical only is handled upstream by graph construction) ---
      gp.g->Draw("E1P SAME");

      // --- acceptance-aware tiered fit based on measured phi-span ---
      // Tier selection:
      //   tier 0: M0 only                         sigma(phi) = M0
      //   tier 1: M0 + A cos(phi)                 sigma(phi) = M0*(1 + A cos(phi))
      //   tier 2: M0 + A cos(phi) + B cos(2phi)   sigma(phi) = M0*(1 + A cos(phi) + B cos(2phi))

      const double BAD   = -999.0;
      const int    BAD_I = -999;

      double sigma0 = BAD, sigma0_err = BAD;
      double A      = BAD, A_err      = BAD;
      double B      = BAD, B_err      = BAD;
      double chi2   = BAD; int ndf    = BAD_I; double prob = BAD;
      int fitStatus = 999;
      int fitTier   = -1;

      const int nPts = gp.g->GetN();
      double phiSpan = 0.0;
      double xmin = 0.0, xmax = 0.0;

      if (nPts >= 1) {
        // compute measured span + mean
        double xmn =  1e99, xmx = -1e99;
        double ysum = 0.0;
        double yMin =  1e99, yMax = -1e99;
        for (int i = 0; i < nPts; i++) {
          double xx, yy;
          gp.g->GetPoint(i, xx, yy);
          xmn = std::min(xmn, xx);
          xmx = std::max(xmx, xx);
          ysum += yy;
        ...
        xmin = xmn; xmax = xmx;
        phiSpan = (xmax > xmin) ? (xmax - xmin) : 0.0;
        const double ymean = ysum / nPts;

        // choose tier by phiSpan
        if (phiSpan < 1.0)              fitTier = 0;
        else if (phiSpan < TMath::Pi()) fitTier = 1;
        else                            fitTier = 2;

        // downgrade if not enough points to constrain parameters (want ndf>=1 when possible)
        // tier2 has 3 params => ndf = N-3, so need N>=4 for ndf>=1
        // tier1 has 2 params => ndf = N-2, so need N>=3 for ndf>=1
        if (fitTier == 2 && nPts < 4) fitTier = (nPts >= 3) ? 1 : 0;
        if (fitTier == 1 && nPts < 3) fitTier = 0;

        TF1* f = nullptr;
        if (fitTier == 0) {
          f = new TF1(Form("f0_pt%d_z%d", iPt, gp.zBin), "[0]", xmin, xmax);
          f->SetParameters(ymean);
          f->SetParLimits(0, 0.0, 1e9);
        } else if (fitTier == 1) {
          f = new TF1(Form("f1_pt%d_z%d", iPt, gp.zBin), "[0]*(1 + [1]*cos(x))", xmin, xmax);
          double A0 = 0.0;
          if (yMax + yMin > 0) {
          ...
          f = new TF1(Form("f2_pt%d_z%d", iPt, gp.zBin), "[0]*(1 + [1]*cos(x) + [2]*cos(2*x))", xmin, xmax);
          double A0 = 0.0;
          if (yMax + yMin > 0) {
            A0 = (yMax - yMin) / (yMax + yMin);
            A0 = std::max(-0.9, std::min(0.9, A0));
        ...
        fitStatus = gp.g->Fit(f, "RQ0");

        sigma0     = f->GetParameter(0);
        sigma0_err = f->GetParError(0);
        if (fitTier >= 1) {
          A     = f->GetParameter(1);
        ...
        if (fitTier >= 2) {
          B     = f->GetParameter(2);
          B_err = f->GetParError(2);
        }
        chi2 = f->GetChisquare();
        ndf  = f->GetNDF();
        prob = f->GetProb();

        // sanitize (avoid NaN/inf in CSV)
        auto sane = [&](double v){ return std::isfinite(v) ? v : BAD; };
        sigma0 = sane(sigma0); sigma0_err = sane(sigma0_err);
        A      = sane(A);      A_err      = sane(A_err);
        B      = sane(B);      B_err      = sane(B_err);
        chi2   = sane(chi2);
        prob   = sane(prob);
        ...
        log << "[fit-tier] pt_bin=" << iPt << " z_bin=" << gp.zBin
            << " N=" << nPts << " phiSpan=" << phiSpan
            << " tier=" << fitTier << " fitStatus=" << fitStatus << "\n";
      } else {
        log << "[fit-skip] pt_bin=" << iPt << " z_bin=" << gp.zBin << " N=0 -> no fit\n";
      }

      std::cout << Form("pt[%d] z[%d]: tier=%d M0=%.3g A=%.3g B=%.3g chi2/ndf=%.2f prob=%.3g\n",
                        gp.ptBin, gp.zBin, fitTier,
                        sigma0, A, B,
                        (ndf > 0 ? chi2 / (double)ndf : -999.0),
                        prob);

      if (saveTables && fitcsv.is_open()) {
        const double ptCtr = 0.5 * (ptLo + ptHi);
        const double zCtr  = 0.5 * (zLo + zHi);
        fitcsv
          << "single" << ","            // mode
          << "" << ","                  // group_id
          ...
          << A << "," << A_err << ","
          << B << "," << B_err << ","
          << chi2 << "," << ndf << "," << prob << "," << fitTier
          << "\n";
      }

      // Legend entry per z-bin (single-mode)
  ...
  for (int iPt=0; iPt<nPt; iPt++) {
    ...
    for (int ic=0; ic<nCurves; ic++) {
      ...
      // Fit: sigma(phi) = M0 * (1 + A cos(phi) + B cos(2phi))
        ...
        const double ymean   = (nPts > 0) ? (ysum / nPts) : 0.0;
        const double phiSpan = (xmax > xmin) ? (xmax - xmin) : 0.0;
        ...
        if (nPts >= 1) {
          ...
          // downgrade by point count for ndf>=1 where possible
          if (fitTier == 2 && nPts < 4) fitTier = (nPts >= 3) ? 1 : 0;
          if (fitTier == 1 && nPts < 3) fitTier = 0;

          TF1* f = nullptr;
          if (fitTier == 0) {
            f = new TF1(Form("f0_%s_pt%d", cv.label.c_str(), iPt), "[0]", xmin, xmax);
            f->SetParameters(ymean);
            f->SetParLimits(0, 0.0, 1e9);
          } else if (fitTier == 1) {
            f = new TF1(Form("f1_%s_pt%d", cv.label.c_str(), iPt), "[0]*(1 + [1]*cos(x))", xmin, xmax);
            double A0 = 0.0;
            if (yMax + yMin > 0) {
              A0 = (yMax - yMin) / (yMax + yMin);
              A0 = std::max(-0.9, std::min(0.9, A0));
            ...
            f = new TF1(Form("f2_%s_pt%d", cv.label.c_str(), iPt), "[0]*(1 + [1]*cos(x) + [2]*cos(2*x))", xmin, xmax);
            double A0 = 0.0;
            if (yMax + yMin > 0) {
              A0 = (yMax - yMin) / (yMax + yMin);
              A0 = std::max(-0.9, std::min(0.9, A0));
          ...
          log << "FIT group group_id=" << groupId
              ...
              << " tier=" << fitTier
              << " fitStatus=" << fitStatus
              << " M0=" << sigma0 << " +/- " << sigma0_err
              << " A="  << A      << " +/- " << A_err
              << " B="  << B      << " +/- " << B_err
              << " chi2=" << chi2 << " ndf=" << ndf << " prob=" << prob
              << "\n";

          if (fitcsv.is_open()) {
            fitcsv
              << "group" << "," << groupId << "," << cv.label << "," << cv.settingId << ","
              << iPt << "," << -1 << ","
              << ptLo << "," << ptHi << "," << ptCtr << ","
