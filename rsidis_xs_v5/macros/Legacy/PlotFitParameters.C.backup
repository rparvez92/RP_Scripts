...
//
// Output:
//   results/<setting_id or group_id>/PNGs/FitParamPlots/*.png
//
...
static std::string InferOutDir(const std::string& csvPath) {
  ...
  // Replace /tables/<file>.csv -> /PNGs/FitParamPlots/
  auto pos = out.rfind("/tables/");
  if (pos != std::string::npos) {
    out = out.substr(0, pos) + "/PNGs/FitParamPlots";
  } else {
    // fallback: sibling PNGs
    out = "PNGs/FitParamPlots";
  }
