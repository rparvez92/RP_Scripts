...
void PlotCoinXsec(const char* manifestPath, const char* outputDir)
{
  gROOT->SetBatch(kTRUE);

  const bool makeComparisonRatio = false;
  const bool makeXsec = true;
  ...
  for (const auto& v : vars) {
    ...
    auto hsim = BuildSimHist(v.simVar, tSim, name, v.nbins, v.xmin, v.xmax, sim_delta_cuts, sim_norm_cuts);
    if (hsim) {
