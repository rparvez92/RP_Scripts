/*
  ...
  dpe_b1 through dpe_b5, dpp_b1 through dpp_b5, and the corresponding full
  ...
  Run examples:
    root -l -b -q 'macros/SimultaneousDpeDpp.C("results/tables/OffsetsSummaryBySetting.csv","","unconstrained")'
    root -l -b -q 'macros/SimultaneousDpeDpp.C("results/tables/OffsetsSummaryBySetting.csv","","constrained")'
*/
...
static bool RunSettingFit(const std::string& setting,
  ...
  for (int i = 0; i < nbin; ++i) {
    ...
    gFitContext.dpp_param_index_by_bin[i] = 1 + nbin + i;
  }
  ...
  for (int ibin = 0; ibin < nbin; ++ibin) {
    ...
    if (setting == "A") {
      ...
      bout.dPmz_pred = -5.75  * res.dthe +  4.10 * bout.dpe + 2.27 * bout.dpp;
      ...
      bout.dPmz_pred = -4.30  * res.dthe +  3.69 * bout.dpe + 3.74 * bout.dpp;
...
static void WriteHeader(std::ofstream& fout, const std::vector<std::string>& param_names) {
  ...
  for (const auto& name : param_names) {
    if (name == "dthe" || name == "dthp") continue;
    fout << name << "_fit," << name << "_fit_err,";
  }
  fout << "dthe_fit,dthe_fit_err,dthp_fit,dthp_fit_err,";
...
void SimultaneousDpeDpp(const char* inCsv = "results/tables/OffsetsSummaryBySetting.csv",
                        const char* outCsv = "",
  ...
  if (outPath.empty()) {
    outPath = "results/tables/SimultaneousDpeDpp_" + mode + ".csv";
  }
