// MeasuredOffsetsByRun.C
//
// Purpose:
//   For one run, or for the hardcoded list of 12 four-pass HEEP runs, compare
//   Data and Sim in SHMS dp bins and measure the offset in fitted Gaussian means
//   for:
//     - W
//     - Em
//     - Pmz
//     - Pmy
//
// Per run, the macro builds 6 dp-bin canvases:
...
//       dmu = mu_data - mu_sim
//   - Fit-derived offsets are written in both GeV and MeV forms where applicable
//
// Output PNGs:
//   results/PNGs/MeasuredOffsetsByRun/offsets_<run>_<binlabel>bin.png
//
// Output CSV:
//   results/tables/MeasuredOffsetsByRun.csv
//
// Run from:
//   RP_Scripts/heep_check_v1/
//
// Examples:
//   Single run:
//     root -l -b -q 'MeasuredOffsetsByRun.C+(23839,-2,3)'
//
//   Loop over the hardcoded 12 runs:
//     root -l -b -q 'MeasuredOffsetsByRun.C+(0,-2,3,5,true)'
...
static const TString kSimDir  = "/home/cdaq/users/rparvez/simc_gfortran/worksim/heep_check_v1/";
...
static void ProcessOneRun(int run,
  ...
  for (const auto& dp : dpBins) {
    ...
    // Save PNG
    TString outName = outDir + Form("/offsets_%d_%sbin.png", run, dp.label.Data());
    c->SaveAs(outName);
...
// Public entry point
// -----------------------------------------------------------------------------
void MeasuredOffsetsByRun(int run = 0,
                          double dpFocusLo = -2.0,
                          double dpFocusHi =  3.0,
                          int nFocusBins = 5,
                          bool loopRuns = false,
                          const char* outDirC = "results/PNGs/MeasuredOffsetsByRun",
                          const char* csvPathC = "results/tables/MeasuredOffsetsByRun.csv") {
