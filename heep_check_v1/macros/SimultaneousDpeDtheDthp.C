/*
  ...
  dthe_b1 through dthe_b5, dpe_b1 through dpe_b5, dthp_b1 through dthp_b5,
  and the corresponding full covariance block; fields not used by a given
  setting are written as nan.
  ...
  Run examples:
    root -l -b -q 'macros/SimultaneousDpeDtheDthp.C("results/tables/OffsetsSummaryBySetting.csv","","unconstrained")'
    root -l -b -q 'macros/SimultaneousDpeDtheDthp.C("results/tables/OffsetsSummaryBySetting.csv","","constrained")'
*/
...
static void SimultaneousChi2Fcn(Int_t& /*npar*/, Double_t* /*grad*/, Double_t& fval,
  ...
  double chi2 = 0.0;
  const int idx_dpp = static_cast<int>(gFitContext.dthe_param_index_by_bin.size()) * 3;
...
static bool RunSettingFit(const std::string& setting,
  ...
  if (constrained) {
    bool onLimit = atBound(res.dpp);
    for (int i = 0; i < npar - 1; ++i) onLimit = onLimit || atBound(res.params[i]);
  ...
  for (int ibin = 0; ibin < nbin; ++ibin) {
    ...
    if (setting == "A") {
      ...
      bout.dPmz_pred = -5.75  * bout.dthe +  4.10 * bout.dpe + 2.27 * res.dpp;
      ...
      bout.dPmz_pred = -4.30  * bout.dthe +  3.69 * bout.dpe + 3.74 * res.dpp;
...
static void WriteHeader(std::ofstream& fout, const std::vector<std::string>& param_names) {
  ...
  for (const auto& name : param_names) {
    if (name == "dpp") continue;
    fout << name << "_fit," << name << "_fit_err,";
  }
  fout << "dpp_fit,dpp_fit_err,";
...
void SimultaneousDpeDtheDthp(const char* inCsv = "results/tables/OffsetsSummaryBySetting.csv",
                             const char* outCsv = "",
  ...
  if (outPath.empty()) {
    outPath = "results/tables/SimultaneousDpeDtheDthp_" + mode + ".csv";
  }
