/*
  ...
  ---------
    ...
    Settings C-I: accept the available valid narrow bins (for 5-pass typically b1 only)
  ...
  Run examples:
    root -l -b -q 'macros/SimultaneousDpeOnly.C("results/tables/MeasuredOffsetsBySetting_5pass.csv","")'
*/
...
static void MinuitFCN(Int_t& /*npar*/, Double_t* /*grad*/, Double_t& fval,
  ...
  std::vector<double> pars(static_cast<size_t>(gFitContext.dpe_param_index_by_bin.size()) + 3, 0.0);
  for (int i = 0; i < static_cast<int>(pars.size()); ++i) pars[i] = par[i];
...
static bool BuildSettingEquations(const std::string& setting,
  ...
  if (setting == "A" || setting == "B") {
    ...
    for (const auto& bin : desired) {
      auto it = byBin.find(bin);
      if (it == byBin.end()) continue;
      const BinData& b = it->second;
      if (!(b.W.ok && b.Em.ok && b.Pmz.ok && b.Pmy.ok)) continue;
      bins.push_back(bin);
    }
    if (bins.empty()) {
      note = "no_valid_narrow_bins_for_setting";
      return false;
    }
  }
...
static bool RunSettingFit(const std::string& setting,
  ...
  for (int ibin = 0; ibin < static_cast<int>(bins.size()); ++ibin) {
    ...
    bout.dW_pred   = c.W_dthe   * res.dthe + c.W_dpe   * bout.dpe + c.W_dthp   * res.dthp + c.W_dpp   * res.dpp;
    bout.dEm_pred  = c.Em_dthe  * res.dthe + c.Em_dpe  * bout.dpe + c.Em_dthp  * res.dthp + c.Em_dpp  * res.dpp;
    bout.dPmz_pred = c.Pmz_dthe * res.dthe + c.Pmz_dpe * bout.dpe + c.Pmz_dthp * res.dthp + c.Pmz_dpp * res.dpp;
    bout.dPmy_pred = c.Pmy_dthe * res.dthe + c.Pmy_dpe * bout.dpe + c.Pmy_dthp * res.dthp + c.Pmy_dpp * res.dpp;
...
void SimultaneousDpeOnly(const char* inCsv = "results/tables/MeasuredOffsetsBySetting_5pass.csv",
                         const char* outCsv = "") {
  ...
  if (outPath.empty()) {
    outPath = "results/tables/SimultaneousDpeOnly_5pass.csv";
  }
