...
offsets, with pass-dependent beam-energy offsets de0_4pass and de0_5pass.
...
Outputs:
  ...
  results/tables/Combined4and5PassWithDe0PassDep/Combined4and5PassResiduals_WithDe0PassDep.csv
...
Run from heep_check_v3:
  python3 macros/Combined4and5PassFitParams_WithDe0PassDep.py
"""
...
OPT_OPTIONS = {"maxiter": 70000, "maxfev": 120000, "xatol": 1e-10, "fatol": 1e-10}
...
def param_vector(params: np.ndarray, row: ObsRow) -> np.ndarray:
    return np.array(
        [
            params[PARAM_INDEX[de0_name(row)]],
...
def write_outputs(rows: list[ObsRow], result, out_dir: Path) -> None:
    ...
    fitparams_out = out_dir / "Combined4and5PassFitParams_WithDe0PassDep.csv"
    residuals_out = out_dir / "Combined4and5PassResiduals_WithDe0PassDep.csv"
    ...
    with fitparams_out.open("w", newline="") as f:
        ...
        writer.writerow(
            [
                "combined_4and5_pass_de0_global_dthp",
    ...
    print(f"[INFO] combined 4+5 with pass-dependent de0 chi2/ndf = {chi2_ndf:.4f}")
