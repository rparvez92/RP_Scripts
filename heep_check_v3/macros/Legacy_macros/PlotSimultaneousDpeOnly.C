// PlotSimultaneousDpeOnly.C
//
// Read a simultaneous-fit CSV produced by:
//   macros/SimultaneousDpeOnly.C
//
// and produce, for each setting, three PNGs:
//
//   results/PNGs/PlotSimultaneousDpeOnly/setting<label>_kinOffsets.png
//   results/PNGs/PlotSimultaneousDpeOnly/setting<label>_residuals.png
//   results/PNGs/PlotSimultaneousDpeOnly/setting<label>_chi2ndf.png
//
// Plot types:
...
//   - the CSV stores bin-specific dpe columns (for example dpe_b1_fit),
//     which are mapped here onto the plotted dpe series row-by-row
//   - for 5-pass data, the x-axis uses the measured dp bin center from the
//     CSV dp_lo/dp_hi columns rather than fixed 4-pass label centers
//
// Default paths:
//   results/tables/SimultaneousDpeOnly_5pass.csv
//   results/PNGs/PlotSimultaneousDpeOnly_5pass
//
// Example:
//   root -l -b -q 'macros/PlotSimultaneousDpeOnly.C()'
//   root -l -b -q 'macros/PlotSimultaneousDpeOnly.C("results/tables/SimultaneousDpeOnly_5pass.csv","results/PNGs/PlotSimultaneousDpeOnly_5pass")'
//
// This v3 copy is intended to be run from:
...
static double BinCenterFromRow(const Row& r) {
  if (std::isfinite(r.dp_lo) && std::isfinite(r.dp_hi)) {
    return 0.5 * (r.dp_lo + r.dp_hi);
  }
  return BinCenterFromLabel(r.bin);
}

static void ComputeXRange(const std::vector<Row>& rows,
                          double& xMin,
                          double& xMax) {
  xMin = std::numeric_limits<double>::infinity();
  xMax = -std::numeric_limits<double>::infinity();
  for (const auto& r : rows) {
    const double xc = BinCenterFromRow(r);
  ...
  if (std::fabs(xMax - xMin) < 1e-12) {
    const double pad = (std::fabs(xMax) > 0.0) ? 0.2 * std::fabs(xMax) : 1.0;
    xMin -= pad;
    xMax += pad;
  } else {
    const double pad = 0.18 * (xMax - xMin);
    xMin -= pad;
    xMax += pad;
  }
}
...
static double GetChi2PlotValue(const Row& r) {
  if (std::isfinite(r.chi2_ndf)) return r.chi2_ndf;
  return r.chi2_min;
}

static std::string GetChi2PlotLabel(const Row& r) {
  if (std::isfinite(r.chi2_ndf)) return "chi2/ndf";
  return "chi2";
}
...
static std::vector<Row> ReadSimultaneousCsv(const std::string& csvPath) {
  ...
  const int iDthe      = idx("dthe_fit");
  const int iDtheErr   = idx("dthe_fit_err");
  const int iDthp      = idx("dthp_fit");
  const int iDthpErr   = idx("dthp_fit_err");
  ...
  const int iChi2Min   = idx("chi2_min");
  ...
  need(iBin, "bin");
  need(iDthe, "dthe_fit");
  need(iDtheErr, "dthe_fit_err");
  need(iDthp, "dthp_fit");
  need(iDthpErr, "dthp_fit_err");
  need(iDpp, "dpp_fit");
  ...
  while (std::getline(fin, line)) {
    ...
    r.bin           = getS(iBin);
    r.dthe_fit      = getD(iDthe);
    r.dthe_fit_err  = getD(iDtheErr);
    r.dthp_fit      = getD(iDthp);
    r.dthp_fit_err  = getD(iDthpErr);
    r.dpp_fit       = getD(iDpp);
    ...
    r.chi2_min      = getD(iChi2Min);
    r.chi2_ndf      = getD(iChi2ndf);
    r.fit_valid     = getI(iFitValid);

    const std::string dpeCol = "dpe_" + Lower(Trim(r.bin)) + "_fit";
    const std::string dpeErrCol = "dpe_" + Lower(Trim(r.bin)) + "_fit_err";
    r.dpe_fit = getD(idx(dpeCol));
    r.dpe_fit_err = getD(idx(dpeErrCol));
...
static void DrawSeriesPad(TPad* pad,
  ...
  ComputeXRange(sorted, xMin, xMax);
...
static void MakeOffsetsCanvas(const std::string& setting,
  ...
  TCanvas* c = new TCanvas(Form("c_sim_kin_%s", setting.c_str()),
                           "SimultaneousDpeOnly kinematic offsets", 1200, 900);
...
static void MakeResidualsCanvas(const std::string& setting,
                                const std::vector<Row>& rows,
                                const std::string& outDir) {
  TCanvas* c = new TCanvas(Form("c_sim_res_%s", setting.c_str()),
                           "SimultaneousDpeOnly residuals", 1200, 900);
  c->Divide(2,2,0.002,0.002);
...
static void MakeChi2Canvas(const std::string& setting,
                           const std::vector<Row>& rows,
                           const std::string& outDir) {
  TCanvas* c = new TCanvas(Form("c_sim_chi2_%s", setting.c_str()),
                           "SimultaneousDpeOnly chi2/ndf", 900, 700);
  c->cd();
  gPad->SetGrid(1,1);
  gPad->SetTicks(1,1);
  ...
  for (const auto& r : sorted) {
    const double xc = BinCenterFromRow(r);
    const double yy = GetChi2PlotValue(r);
    if (!std::isfinite(xc) || !std::isfinite(yy)) continue;
    x.push_back(xc);
    y.push_back(yy);
    if (std::isfinite(r.chi2_ndf)) useNdf = true;
  }
  ...
  if (!y.empty()) {
    ...
    if (std::fabs(yMax - yMin) < 1e-12) {
      const double pad = (std::fabs(yMax) > 0.0) ? 0.18 * std::fabs(yMax) : 1.0;
      yMin -= pad;
      yMax += pad;
    } else {
  ...
  g->SetLineWidth(2);
  g->GetXaxis()->SetTitle("#delta bin center (%)");
  g->GetYaxis()->SetTitle(useNdf ? "chi2/ndf" : "chi2");
  g->GetXaxis()->SetLimits(xMin, xMax);
  g->GetYaxis()->SetRangeUser(yMin, yMax);
  g->GetXaxis()->CenterTitle();
  g->GetYaxis()->CenterTitle();
...
void PlotSimultaneousDpeOnly(const char* inCsvIn = "",
                             const char* outDirIn = "") {
  std::string inCsv = Trim(inCsvIn ? inCsvIn : "");
  if (inCsv.empty()) {
    inCsv = "results/tables/SimultaneousDpeOnly_5pass.csv";
  }

  std::string outDir = Trim(outDirIn ? outDirIn : "");
  if (outDir.empty()) {
    outDir = "results/PNGs/PlotSimultaneousDpeOnly_5pass";
  }
  ...
  if (bySetting.empty()) {
    std::cerr << "[ERROR] No valid rows found in " << inCsv << std::endl;
    return;
  }

  if (bySetting.count("A")) {
    MakeOffsetsCanvas("A", bySetting["A"], outDir);
    MakeResidualsCanvas("A", bySetting["A"], outDir);
    MakeChi2Canvas("A", bySetting["A"], outDir);
  }
  if (bySetting.count("B")) {
    MakeOffsetsCanvas("B", bySetting["B"], outDir);
    MakeResidualsCanvas("B", bySetting["B"], outDir);
    MakeChi2Canvas("B", bySetting["B"], outDir);
  }

  for (const auto& kv : bySetting) {
    if (kv.first == "A" || kv.first == "B") continue;
    MakeOffsetsCanvas(kv.first, kv.second, outDir);
    MakeResidualsCanvas(kv.first, kv.second, outDir);
    MakeChi2Canvas(kv.first, kv.second, outDir);
  }

  std::cout << "[INFO] Wrote PlotSimultaneousDpeOnly PNGs to " << outDir
