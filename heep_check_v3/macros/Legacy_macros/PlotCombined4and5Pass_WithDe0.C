// PlotCombined4and5Pass_WithDe0.C
//
// Plot the combined 4+5-pass fit with one global de0, toy errors, and residuals.
//
// Inputs:
//   results/tables/Combined4and5PassWithDe0/Combined4and5PassFitParams_WithDe0.csv
//   results/tables/Combined4and5PassWithDe0/Combined4and5PassErrors_WithDe0_params.csv
//   results/tables/Combined4and5PassWithDe0/Combined4and5PassResiduals_WithDe0_WithToyErrors.csv
//
// Outputs:
//   results/PNGs/Combined4and5PassWithDe0/chi2ndf_WithDe0.png
//   results/PNGs/Combined4and5PassWithDe0/kinOffsets_WithDe0.png
//   results/PNGs/Combined4and5PassWithDe0/residuals_WithDe0.png
//
// Run from heep_check_v3:
//   root -l -b -q 'macros/PlotCombined4and5Pass_WithDe0.C'
...
static FitSummary ReadFit(const TString& path) {
  ...
  for (const auto& h : headers) {
    if (h == "de0" || h.rfind("dthe", 0) == 0 || h.rfind("dpe", 0) == 0 || h.rfind("dpp", 0) == 0 || h == "dthp") {
...
static void PlotChi2(const FitSummary& fit, const TString& out) {
  TCanvas c("c_comb45_de0_chi2", "Combined 4+5 with de0 chi2", 1000, 720);
  c.SetGrid();
  c.SetLeftMargin(0.12);
  c.SetBottomMargin(0.12);
  TH1F* frame = MakeCategoryFrame("h_chi2_frame_de0", "Combined 4+5-pass fit with global de0", "#chi^{2}/ndf", 0, std::max(1.0, 1.35 * fit.chi2ndf));
  frame->Draw();
...
static void PlotKin(const FitSummary& fit, const std::map<std::string, ParamErr>& errs, const TString& out) {
  TCanvas c("c_comb45_de0_kin", "Combined 4+5 kinematic offsets with de0", 1500, 1050);
  c.Divide(3, 2);
  ...
  lat.DrawLatex(0.16, 0.70, "Global parameters:");
  lat.DrawLatex(0.16, 0.58, "#DeltaE_{0} and #Delta#theta_{p}");
  lat.DrawLatex(0.16, 0.46, "drawn at setting C");
  c.SaveAs(out);
}
...
static void PlotResiduals(const std::vector<ResidualRow>& rows, const TString& out) {
  ...
  TCanvas c("c_comb45_de0_residuals", "Combined 4+5 with de0 residuals", 1500, 700);
  c.SetGrid();
  c.SetLeftMargin(0.09);
  c.SetRightMargin(0.03);
  c.SetBottomMargin(0.12);
  TH1F* frame = MakeCategoryFrame("h_resid_frame_de0", "Combined 4+5-pass residuals with global de0", "Residual (MeV)", ymin, ymax);
...
void PlotCombined4and5Pass_WithDe0(const char* tableDirC = "results/tables/Combined4and5PassWithDe0",
                                   const char* outDirC = "results/PNGs/Combined4and5PassWithDe0") {
  ...
  const FitSummary fit = ReadFit("results/tables/Combined4and5PassWithDe0/Combined4and5PassFitParams_WithDe0.csv");
  const auto errs = ReadParamErrors(inDir + "/Combined4and5PassErrors_WithDe0_params.csv", fit);
  const auto residuals = ReadResiduals(inDir + "/Combined4and5PassResiduals_WithDe0_WithToyErrors.csv");

  PlotChi2(fit, outDir + "/chi2ndf_WithDe0.png");
  PlotKin(fit, errs, outDir + "/kinOffsets_WithDe0.png");
  PlotResiduals(residuals, outDir + "/residuals_WithDe0.png");
  std::cout << "[INFO] Wrote " << outDir << "/chi2ndf_WithDe0.png\n";
  std::cout << "[INFO] Wrote " << outDir << "/kinOffsets_WithDe0.png\n";
  std::cout << "[INFO] Wrote " << outDir << "/residuals_WithDe0.png\n";
}
