// PlotCombined4and5Pass_WithDe0PassDep.C
//
// Plot the combined 4+5-pass fit with pass-dependent de0, toy errors, and residuals.
//
// Inputs:
//   results/tables/Combined4and5PassWithDe0PassDep/Combined4and5PassFitParams_WithDe0PassDep.csv
//   results/tables/Combined4and5PassWithDe0PassDep/Combined4and5PassErrors_WithDe0PassDep_params.csv
//   results/tables/Combined4and5PassWithDe0PassDep/Combined4and5PassResiduals_WithDe0PassDep_WithToyErrors.csv
//
// Outputs:
//   results/PNGs/Combined4and5PassWithDe0PassDep/chi2ndf_WithDe0PassDep.png
//   results/PNGs/Combined4and5PassWithDe0PassDep/kinOffsets_WithDe0PassDep.png
//   results/PNGs/Combined4and5PassWithDe0PassDep/residuals_WithDe0PassDep.png
//
// Run from heep_check_v3:
//   root -l -b -q 'macros/PlotCombined4and5Pass_WithDe0PassDep.C'
...
static FitSummary ReadFit(const TString& path) {
  ...
  for (const auto& h : headers) {
    if (h.rfind("de0", 0) == 0 || h.rfind("dthe", 0) == 0 || h.rfind("dpe", 0) == 0 || h.rfind("dpp", 0) == 0 || h == "dthp") {
...
static void PlotChi2(const FitSummary& fit, const TString& out) {
  TCanvas c("c_comb45_de0_passdep_chi2", "Combined 4+5 with pass-dependent de0 chi2", 1000, 720);
  c.SetGrid();
  c.SetLeftMargin(0.12);
  c.SetBottomMargin(0.12);
  TH1F* frame = MakeCategoryFrame("h_chi2_frame_de0_passdep", "Combined 4+5-pass fit with pass-dependent de0", "#chi^{2}/ndf", 0, std::max(1.0, 1.35 * fit.chi2ndf));
  frame->Draw();
...
static void PlotKin(const FitSummary& fit, const std::map<std::string, ParamErr>& errs, const TString& out) {
  TCanvas c("c_comb45_de0_passdep_kin", "Combined 4+5 kinematic offsets with pass-dependent de0", 1500, 1050);
  c.Divide(3, 2);
  const std::vector<double> x5 = {0, 1, 2, 3, 4};
  c.cd(1); gPad->SetGrid(); gPad->SetLeftMargin(0.20); gPad->SetRightMargin(0.04); gPad->SetBottomMargin(0.13);
  DrawParamPad({"de0_4pass", "de0_5pass"}, {0.5, 3.0}, errs, fit,
               "Combined 4+5 Offsets: #DeltaE_{0}", "#DeltaE_{0} (MeV)");
  c.cd(2); gPad->SetGrid(); gPad->SetLeftMargin(0.20); gPad->SetRightMargin(0.04); gPad->SetBottomMargin(0.13);
  DrawParamPad({"dthe_A","dthe_B","dthe_C","dthe_E","dthe_D"}, x5, errs, fit,
               "Combined 4+5 Offsets: #Delta#theta_{e}", "#Delta#theta_{e} (mrad)");
  c.cd(3); gPad->SetGrid(); gPad->SetLeftMargin(0.20); gPad->SetRightMargin(0.04); gPad->SetBottomMargin(0.13);
  DrawParamPad({"dpe_A","dpe_B","dpe_C","dpe_E","dpe_D"}, x5, errs, fit,
               "Combined 4+5 Offsets: #Deltap_{e}", "#Deltap_{e} (0.1%)");
  c.cd(4); gPad->SetGrid(); gPad->SetLeftMargin(0.20); gPad->SetRightMargin(0.04); gPad->SetBottomMargin(0.13);
  DrawParamPad({"dthp"}, {2.0}, errs, fit,
               "Combined 4+5 Offsets: #Delta#theta_{p}", "#Delta#theta_{p} (mrad)");
  c.cd(5); gPad->SetGrid(); gPad->SetLeftMargin(0.20); gPad->SetRightMargin(0.04); gPad->SetBottomMargin(0.13);
  ...
  lat.DrawLatex(0.13, 0.72, "Pass-dependent:");
  lat.DrawLatex(0.13, 0.60, "#DeltaE_{0}^{4pass}, #DeltaE_{0}^{5pass}");
  lat.DrawLatex(0.13, 0.45, "Global:");
  lat.DrawLatex(0.13, 0.33, "#Delta#theta_{p}");
  c.SaveAs(out);
}
...
static void PlotResiduals(const std::vector<ResidualRow>& rows, const TString& out) {
  ...
  TCanvas c("c_comb45_de0_passdep_residuals", "Combined 4+5 with pass-dependent de0 residuals", 1500, 700);
  c.SetGrid();
  c.SetLeftMargin(0.09);
  c.SetRightMargin(0.03);
  c.SetBottomMargin(0.12);
  TH1F* frame = MakeCategoryFrame("h_resid_frame_de0_passdep", "Combined 4+5-pass residuals with pass-dependent de0", "Residual (MeV)", ymin, ymax);
...
void PlotCombined4and5Pass_WithDe0PassDep(const char* tableDirC = "results/tables/Combined4and5PassWithDe0PassDep",
                                          const char* outDirC = "results/PNGs/Combined4and5PassWithDe0PassDep") {
  gROOT->SetBatch(kTRUE);
  gStyle->SetOptStat(0);
  const TString inDir(tableDirC);
  const TString outDir(outDirC);
  if (gSystem->AccessPathName(outDir)) gSystem->mkdir(outDir, kTRUE);

  const FitSummary fit = ReadFit(inDir + "/Combined4and5PassFitParams_WithDe0PassDep.csv");
  const auto errs = ReadParamErrors(inDir + "/Combined4and5PassErrors_WithDe0PassDep_params.csv", fit);
  const auto residuals = ReadResiduals(inDir + "/Combined4and5PassResiduals_WithDe0PassDep_WithToyErrors.csv");

  PlotChi2(fit, outDir + "/chi2ndf_WithDe0PassDep.png");
  PlotKin(fit, errs, outDir + "/kinOffsets_WithDe0PassDep.png");
  PlotResiduals(residuals, outDir + "/residuals_WithDe0PassDep.png");
  std::cout << "[INFO] Wrote " << outDir << "/chi2ndf_WithDe0PassDep.png\n";
  std::cout << "[INFO] Wrote " << outDir << "/kinOffsets_WithDe0PassDep.png\n";
  std::cout << "[INFO] Wrote " << outDir << "/residuals_WithDe0PassDep.png\n";
}
