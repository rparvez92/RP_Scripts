...
//   macros/SimultaneousDpeDthe.C
//
// and produce, for each setting, three PNGs:
//
//   results/PNGs/PlotSimultaneousDpeDthe/setting<label>_kinOffsets.png
//   results/PNGs/PlotSimultaneousDpeDthe/setting<label>_residuals.png
//   results/PNGs/PlotSimultaneousDpeDthe/setting<label>_chi2ndf.png
//
// Plot types:
...
//   - the CSV stores bin-specific dthe and dpe columns (for example dthe_b1_fit,
//     dpe_b1_fit), which are mapped here onto the plotted series row-by-row
//
// Default paths:
//   results/tables/SimultaneousDpeDthe.csv
//   results/PNGs/PlotSimultaneousDpeDthe
//
// Example:
//   root -l -b -q 'macros/PlotSimultaneousDpeDthe.C()'
//   root -l -b -q 'macros/PlotSimultaneousDpeDthe.C("results/tables/SimultaneousDpeDthe.csv","results/PNGs/PlotSimultaneousDpeDthe")'
...
static void MakeOffsetsCanvas(const std::string& setting,
  ...
  TCanvas* c = new TCanvas(Form("c_sim_dpdt_kin_%s", setting.c_str()),
                           "SimultaneousDpeDthe kinematic offsets", 1200, 900);
...
static void MakeResidualsCanvas(const std::string& setting,
                                const std::vector<Row>& rows,
                                const std::string& outDir) {
  TCanvas* c = new TCanvas(Form("c_sim_dpdt_res_%s", setting.c_str()),
                           "SimultaneousDpeDthe residuals", 1200, 900);
  c->Divide(2,2,0.002,0.002);
...
static void MakeChi2Canvas(const std::string& setting,
                           const std::vector<Row>& rows,
                           const std::string& outDir) {
  TCanvas* c = new TCanvas(Form("c_sim_dpdt_chi2_%s", setting.c_str()),
                           "SimultaneousDpeDthe chi2/ndf", 900, 700);
  c->cd();
  gPad->SetGrid(1,1);
  gPad->SetTicks(1,1);
...
void PlotSimultaneousDpeDthe(const char* inCsvIn = "",
                             const char* outDirIn = "") {
  std::string inCsv = Trim(inCsvIn ? inCsvIn : "");
  if (inCsv.empty()) {
    inCsv = "results/tables/SimultaneousDpeDthe.csv";
  }

  std::string outDir = Trim(outDirIn ? outDirIn : "");
  if (outDir.empty()) {
    outDir = "results/PNGs/PlotSimultaneousDpeDthe";
  }
  ...
  std::cout << "[INFO] Wrote PlotSimultaneousDpeDthe PNGs to " << outDir
